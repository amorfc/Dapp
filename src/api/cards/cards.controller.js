...
class CardsController {
  static async addCard(req, res) {
    try {
      ...
      if (!dbResult.success) {
        res
          .status(401)
          .json({ error: "Something went wrong while adding card" });
        return;
      }
  ...
  static async getCard(req, res) {
    try {
      ...
      if (!cardFromDbResult.success) {
        res
          .status(401)
          .json({ error: "There is no card associate with this student " });
        return;
      }
  ...
  static async updateCard(req, res) {
    try {
      ...
      const dbResult = await CardsDao.updateCard(
        authenticatedUser.schoolNo,
        newCardInfoFromBody
      );
      ...
      if (!dbResult.success) {
        res
          .status(401)
          .json({ error: "Something went wrong while updating card" });
        return;
      }
